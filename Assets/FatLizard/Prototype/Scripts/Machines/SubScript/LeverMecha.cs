...
public class LeverMecha : MonoBehaviour
{
		...
		if (Physics.Raycast (rayInfo, out hitInfo))
		{
			if(hitInfo.collider != null)
			{
				if(hitInfo.collider.GetInstanceID().Equals( handleCollider.GetInstanceID() ))
				{
					...
					{
						CustomReference.Access.userInterfaces.noBetDisplay.SetActive(true);
						CustomEvents.OnNotificationEvents (Notification.NoChipsBet); return;
					}
