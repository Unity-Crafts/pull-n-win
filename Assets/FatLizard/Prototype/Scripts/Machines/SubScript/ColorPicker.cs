...
public class ColorPicker : MonoBehaviour
{
	...
	{
		if(hitInfo.transform.GetComponentInParent<ColorPicker>() != null && machine.onReadyPlay)
		{
			if(CustomReference.Access.userInterfaces.userDetails.currentCash > 0)
			{
				BillValue betting = CustomReference.Access.machineGroups.OnActiveChipPrefab;

				if(CustomReference.Access.userInterfaces.userDetails.currentCash >= betting.amount)
				{
					...
					(
						...
						CustomReference.Access.machineGroups.OnActiveChipPrefab.amount //GET THE CURRENT AMOUNT OF CHIP PLACED.
					);

					//SUBCTRACT THE AMOUNT FROM THE TOTAL CASH!
					CustomReference.Access.userInterfaces.userDetails.currentCash -= betting.amount;
					...
					CustomReference.Access.userInterfaces.userRecords.totalBet += betting.amount;
				...
				{
					ColorBlock (true);
					CustomEvents.OnNotificationEvents (Notification.ChipLowerCash);
				}
			}

			else
			{
				if( !CustomReference.Access.objectReferences.offerCoins.GetCurrentAnimatorStateInfo(0).IsTag("Showing") )
				{
					CustomReference.Access.objectReferences.offerCoins.SetTrigger ("Show");
					CustomEvents.OnNotificationEvents (Notification.Insufficient);
				}
			}
		}
	}
	...
	public void ColorBlock(bool activeBlockage)
	{
		if(activeBlockage)
		{
			if(!colorBlock)
			{
				CustomReference.Access.userInterfaces.lowerBetDisplay.SetActive (true);
				colorBlock = true;
			}
		}
		...
		else
		{
			if(colorBlock)
			{
				CustomReference.Access.userInterfaces.lowerBetDisplay.SetActive (false);
				colorBlock = false;
			}
		}
	}
